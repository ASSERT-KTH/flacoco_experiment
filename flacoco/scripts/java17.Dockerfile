...
RUN apt-get update
RUN apt-get install git subversion openjdk-17-jdk curl unzip build-essential cpanminus python3.10 python3.10-distutils -y
...
CMD [ "python3.10", "runAllBugs.py" , "--storage", "projects" , "--output", "results" , "--target" , "17" ]
